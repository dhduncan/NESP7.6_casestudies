...
# Summary 
What is information is required to support cost-effective management of wild herbivores?
...
The threat that multiple herbivores posed to the restoration of semi-arid woodlands in Victoria was recognised by the Land Conservation Council (LCC) in their investigation of public land use in the Mallee region 1972--1977 [@lcc1977]. The LCC report recognised the deficiency in information base about the Mallee ecosystems, flora and fauna, and highlighted the complex problem that the _combined_ grazing pressure from livestock, rabbits, hares, goats and over-abundant kangaroos posed for the degraded semi-arid woodlands. 
...
The LCC called for field studies, and their review of public land use a decade later [@lcc1989] was in a much better position to recommend the termination of livestock grazing leases. They argued, supported by field data and site visits,  that no constant livestock grazing pressure was compatible with the restoration objective.  One herbivore threat down, five to go.

## Semi-arid woodlands
Restoration objectives that could inform the impact side of a density-impact understanding were first set out in 1990 with the release of the Restoring the Balance plan for Kangaroo Control, a package of plans including a kangaroo management plan and a vegetation management plan. 




```{r bar graph of output types, fig.align='center', fig.cap= "Figure 2.  Summary of number of evidence inputs of different types according to primary focus."}
filter(data, focus != "livestock" & focus != "goat") |> 
ggplot(aes(x = dd_mashup)) +
  geom_bar() +
  facet_wrap(~focus) +
  coord_flip() +
  theme_duncan() 
...
For all the rabbit survey and control effort undertaken in the Mallee parks, the understanding about density-impact relationships were largely extrapolated from a developing body of work undertaken in semi-arid woodland and coastal scrub sites in South Australia [@mutze2016, @bird2012]. 
...
Can look at results in naive sense - how is it that they do not have a handle on DIF with so much effort?

sympathetic resource oriented view - areas added to NPs - known to be in a degraded state, with no additional budget. So lots of work needs to be done opporunistically - not recurrent funding. Student projects, national projects, periodic funding - not much support for a long campaign.

Links to Wintle

Although there is an important rsource limitation, but neither is there much evidence of hte progressive shaping of questions taht would lead to the establishment of DIFs.
...
The cessation of livestock grazing rights obviated the need to develop a livestock density or regime-impact understanding, but  many questions about density-impact story for the various unmanaged herbivores remained. 
...
With the benefit of hindsight it is possible to wonder if an overarching 
